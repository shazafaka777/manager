var playa_module = function(access_token){
if(typeof require == "function"){
	var request = require('request');
	var requestSync = require('sync-request');
	var rq = require('prequest');
	var fs = require("fs")
...
uploadgroup:function(user,save_method,data,cb){
	if(vk.debug)console.log("Получаем сервер для загрузки файлов...");
	var _data = vk.other.collect(data,{files:""});
	vk.api.docs.getMessagesUploadServer({peer_id: user}, function(s){
		if(vk.debug){
			console.log("Результаты получения сервера для загрузки...",s);
			console.log("Загрузка файлов...");
		}
	...
	if(fack.match(/null/)) {
		...
		vk.default_method(url,data,function(body){
			if(typeof body.response !== "undefined"){
				...
				else if(body.error.error_code == 7 || body.error.error_code == 15) {
					callback(body);
				}
				else{
					if(cberr == 1) {
...
_forceapi:function(method,data,callback,place){
	if(typeof data !== "object")data = {};
	if(typeof callback !== "function")callback = vk.default_callback;
	if(vk.cart && typeof place == "undefined")place = vk.cart.default_place;
	if(!vk.cart || !place){
	}else if(place == -1){
		return vk.cart.list.unshift([vk._forceapi,[method,data,callback]]);
	}else if(place == 1){
		return vk.cart.list.push([vk._forceapi,[method,data,callback]]);
	}
	var url = vk.api_url + method + "?random=" + Math.random();
	if(vk.access_token && !data["access_token"])data["access_token"] = vk.access_token;
	if(!data["v"])data["v"] = vk.default_version;
...
addListener:{
	messages:function(_cb,data){
		...
		vk.api.messages.getLongPollServer({use_ssl:0,need_pts:1,access_token:data.access_token},function(res){
			...
			ts = res.response.ts
			console.log("Сервер успешно получен.")
			...
			setInterval(function(){
				...
				/*	if(res.response.messages.count != 0) {
						if(res.response.messages.items[0].peer_id == 2000000277) {
						//	console.log(res.response.messages.items)
				...
				//	delete res.response.profiles.map(function(profile){users_info[profile.id] = profile;});
					delete res.response.messages.items.map(function(msg){
						...
						for(var i = 0; i < res.response.conversations.length; i++) {
							if(res.response.conversations[i].peer.local_id == msg.chat_id) {
								msg.title = res.response.conversations[i].chat_settings.title
							}
						}
						if(msg.from_id > 0) {
							res.response.profiles.map(function(profile) {
								if(profile.id == msg.from_id) {
									msg.user_info = profile
								}
							})
						}
					//	msg.user_info = (users_info[msg.from_id] || {});
...
on:{
	...
	error:function(_error,_m,_q,_cb){
			if(_error.error_code == 6){
				vk._api(_m,_q,_cb,-1);
			}else if(_error.error_code == 14){
...
vk.default_method = browser?vk.sreq.jsonp:vk.sreq.post;
vk.default_syncMethod = browser?vk.sreq.ePostSync:vk.sreq.postSync;
vk.apiSync = {};
vk.api = {
	upload: {
...
//	if(!vk.forceapi[m.split(".")[0]])vk.forceapi[m.split(".")[0]] = {};
...
/*	vk.forceapi[m.split(".")[0]][m.split(".")[1]] = (function(method){
		return function(data,callback,place){
			return vk._forceapi(method,data,callback,place);
		}
	}
	)(m);*/
